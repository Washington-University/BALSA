...
abstract class AbstractDatasetController extends AbstractBalsaController {
	...
	def downloadStats() {
		...
		render (template: "/templates/downloadStatsModalContents", model: [downloadStats: statsService.getDownloadStats(20, datasetInstance)])
	}
	...
	def saveSceneEdits() {
		...
		for (sceneFile in datasetInstance.workingVersion().sceneFiles()) {
			sceneFile.label = params[sceneFile.id + 'label']
			for (scene in sceneFile.scenes) {
				scene.shortName = params[scene.id + 'shortName']
				def sceneTags = params[scene.id + 'tags'] ?: []
				if (sceneTags instanceof String) sceneTags = [sceneTags]
				scene.tags = sceneTags as String[] // needs to handle blank arrays and single item arrays
			}
		}
	...
	def updateNotes() {
		...
		datasetInstance.readNotes = springSecurityService.currentUser.username
		...
		if (workingVersion.status == Version.Status.NONPUBLIC) {
			...
			if (datasetInstance instanceof Study) {
				if (!workingVersion.submitter) {
					workingVersion.submitter = springSecurityService.currentUser
				...
				if (Environment.current == Environment.PRODUCTION) {
					mailService.sendMail {
						...
						html 'Your BALSA study, <a href="' + grailsLinkGenerator.link(action: 'show', id: datasetInstance.id, absolute: true) +
							'">' + (datasetInstance.shortTitle ?: datasetInstance.title) +
							'</a> has been approved.' +
							(submittedVersion.actualReleaseDate() ? (' It will be made public as of its release date: ' + submittedVersion.actualReleaseDate().format("yyyy-MM-dd h:mm a z") + '.') : '')
					}
				}
			}
		}
	...
	def messageCurator() {
		...
		if (Environment.current == Environment.PRODUCTION) {
			mailService.sendMail {
				...
				html springSecurityService.currentUser.username + ' sent the following message to the BALSA curator(s) of study "<a href="balsa.wustl.edu/' + 
	...
	def addLinkedScene() {
		...
		datasetInstance.addToLinkedScenes(scene)
	...
	def removeLinkedScene() {
		...
		datasetInstance.removeFromLinkedScenes(scene)
	...
	def issues() {
		...
		if (datasetInstance instanceof Study) {
			if (!versionInstance.actualReleaseDate()) {
	...
	def createIssue() {
		...
		issue.createdBy = springSecurityService.currentUser.username
		...
		comment.createdBy = springSecurityService.currentUser.username
